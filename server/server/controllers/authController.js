...
exports.login = cathAsync(async (req, res, next) => {
  ...
  //1.cheak if email and password exist
  if (!email || !password) {

    return next(new AppError('Please provide email and password'), 400)
  }
  //2. cheak if user exists and password is correct
...
exports.protect = cathAsync(async (req, res, next) => {
  //1.Getting token and cheak if its there
  let token
  ...
  const decoded = await promisify(jwt.verify)(token, process.env.JWT_SECRET, console.log(token))
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(new AppError('You do not have premission to prefrom this action', 403))
    }
    next()
  }
...
exports.resetPassword = cathAsync(async (req, res, next) => {
  ...
  //2.If token no expired, and there is user,set new password
  if (!user) {
